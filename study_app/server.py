...
def create_app() -> FastAPI:
    ...
    async def get_document(
        ...
        try:
            document = repo.get_document(user, document_id)
    ...
    async def delete_document(
        ...
        repo.delete_document(user, document_id)
    ...
    async def planner(
        ...
        documents = repo.list_documents(user)
        tests = repo.list_assessments(user)
        plan = generate_study_plan(profile, documents, tests, days=max(1, min(days, 30)))
        return plan
