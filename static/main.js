...
function setWorkspaceEnabled(enabled) {
  ...
  if (!enabled) {
    document.getElementById('plannerList').innerHTML =
      '<li class="muted">Sign in to see your tailored schedule.</li>';
    documentPreview.textContent = 'Select a document to read an excerpt.';
  }
}
...
function resetWorkspace() {
  ...
  document.getElementById('plannerList').innerHTML = '<li class="muted">Sign in to see your tailored schedule.</li>';
  documentPreview.textContent = 'Select a document to read an excerpt.';
}
...
async function refreshDocuments() {
  ...
  if (!data.documents.length) {
    list.innerHTML = '<li class="muted">No documents yet. Add lecture notes or handouts.</li>';
    documentPreview.textContent = 'Select a document to read an excerpt.';
    return;
  }
  for (const doc of data.documents) {
    ...
    title.innerHTML = `<strong>${doc.filename}</strong><br/><span>${doc.word_count} words</span>`;
    header.append(title);
    ...
    deleteBtn.addEventListener('click', async (event) => {
      event.stopPropagation();
      if (window.confirm('Remove this document from StudyMate?')) {
    ...
    if (doc.topics?.length) {
      ...
      topicLine.textContent = `Topics: ${doc.topics.join(', ')}`;
...
async function refreshPlanner() {
  ...
  list.innerHTML = '';
  const data = await fetchJSON('/api/planner');
  const summaryItem = document.createElement('li');
  summaryItem.innerHTML = `Daily focus: <strong>${data.summary.daily_minutes}</strong> minutes · Tone: ${data.summary.tone}`;
  list.append(summaryItem);
  for (const block of data.blocks) {
    const item = document.createElement('li');
    item.innerHTML = `
      <strong>${new Date(block.date).toLocaleDateString(undefined, {
        weekday: 'short',
        day: 'numeric',
        month: 'short',
      })}</strong>
      <span>Deep focus (${block.deep_focus_minutes} min): ${block.focus_topic}</span>
      <span>Recap (${block.recap_minutes} min): ${block.review_topic}</span>
      <span class="muted">${block.checkpoint}</span>
    `;
    list.append(item);
  }
}
...
async function openDocument(documentId) {
  documentPreview.textContent = 'Loading document…';
  try {
    const data = await fetchJSON(`/api/documents/${documentId}`);
    documentPreview.dataset.documentId = String(documentId);
    const excerpt = data.document.content.slice(0, 2000);
    documentPreview.textContent = excerpt || 'Document is empty.';
  } catch (error) {
    console.error(error);
    documentPreview.textContent = 'Unable to load document.';
  }
}

async function deleteDocument(documentId) {
  const current = documentPreview.dataset.documentId;
  await fetchJSON(`/api/documents/${documentId}`, { method: 'DELETE' });
  if (current === String(documentId)) {
    delete documentPreview.dataset.documentId;
    documentPreview.textContent = 'Select a document to read an excerpt.';
  }
  await refreshDocuments();
  await refreshInsights();
...
async function bootstrapWorkspace() {
  await Promise.all([loadProfile(), refreshDocuments(), refreshTests(), refreshInsights(), refreshPlanner()]);
  setStatus(true);
}
